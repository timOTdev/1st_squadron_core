...
- [rust_minigrep repo](https://github.com/timOTdev/rust_minigrep)
...
- See [Repo](https://github.com/timOTdev/rust_cargo_workspaces).
...
### Reasons in choosing Box, Ref Counting, or RefCell SPs
...
### using RefCell to solve our predicament
...
- _Reference Cycles_ refers to when there are 2 pieces of data on the heap that each point to each other.
  ...
  - At the end of the program, both data pieces on the heap will not get cleaned up creating a _memory leak_, even if both pointers on the heap have been deallocated.
  - They both reference each other so it's clear that Rust doesn't want to break that on the Heap.

## Basic example of reference cycle
...
// Here, we make a ref counter on the tail for the list.
// The RefCell allows you the change the cell, even though Rc is immutable.
#[derive(Debug)]
enum List {
...
impl List {
  // This method easily allows us to grab the tail end
...
fn main() {
  // Make a Ref Counter with the Cons Structure.
  // Inside the Cons, it has a head of 5 and a tail of RefCell<Rc<List>>.
  let a = Rc::new(Cons(5, RefCell::new(Rc::new(Nil))));

  // We just created variable a so it should be 1.
  // a.tail() grabs the tail for a.
  println!("a initial rc count = {}", Rc::strong_count(&a));
  println!("a next item = {:?}", a.tail());

  // Same thing as variable a but it has tail of reference a.
  // We also cloned it here because we want ownership.
  let b = Rc::new(Cons(10, RefCell::new(Rc::clone(&a))));

  // We just referenced variable a in the tail of b. Count is now 2.
  // Variable b has a count of 1 since we just created it.
  // Tail of b is the Cons of 10 and tail of variable a.
  println!("a rc count after b creation = {}", Rc::strong_count(&a));
  println!("b initial rc count = {}", Rc::strong_count(&b));
  println!("b next item = {:?}", b.tail());

  // We only care about the Some variant of the tail function.
  // If it returns Some, deref it and borrow the mutable data.
  // We can then mutate it and set it as the reference to Variable b.
  // Our Reference cycle now has both variables pointing at each other.
  if let Some(link) = a.tail() {
    *link.borrow_mut() = Rc::clone(&b);
  }

  // Rc for variable b is now 2.
  // Rc for variable a is still 2.
  println!("b rc count after changing a = {}", Rc::strong_count(&b));
  println!("a rc count after changing a = {}", Rc::strong_count(&a));

  // Uncomment the next line to see that we have a cycle;
  // it will overflow the stack.
  // Calling a.tail() points to variable b, but variable b's tail
  // points to variable a, and we get the stack overflow.
  // println!("a next item = {:?}", a.tail());
}
```

## Example of tree structure and nodes

- `Weak` is a version of Rc that holds a non-owning reference to the managed allocation.
- It's allocation can be accessed by calling `upgrade` and returns an `Option<Rc<T>>`.
- `downgrade` converts a Rc smart pointer to a `Weak` smart pointer.

```rust
use std::cell::RefCell;
use std::rc::{Rc, Weak};

// Create a node that's like a Linked List.
// We don't want to use Rc with parent because can create a reference cycle.
// We can just use a Weak smart pointer instead, think like a "weak bond".
// When the child is removed, the parent remains and the weak bond is broken.
#[derive(Debug)]
struct Node {
    value: i32,
...
fn main() {
    // The child tail doesn't lead anywhere.
    ...
    // We only need to borrow here because we won't mutate.
    // upgrade() will make `Weak` SP into `Rc` SP so we can get Option.
    // Because the value might have been dropped, we get Some or None.
    // That's why we need to check with upgrade().
    println!("leaf parent = {:?}", leaf.parent.borrow().upgrade());

    // The parent tail leads to the child node.
    ...
    // *leaf.parent.borrow_mut() let's us mutate the Weak SP.
    // Rc::downgrade makes `Rc` into `Weak` so it can be stored on
    // leaf's parent member.
...
- Rc hold 2 different references. Strong counts references that has ownership, while Weak holds count that does NOT have ownership.
- All the logic for all this is contained within Rc and Weak, specifically the drop trait.
- `strong_count`
- `weak_count`
- I think looking at `Rc::new` and `Weak::new` is how you find them.
...
fn main() {
    ...
    println!(
        "leaf strong = {}, weak = {}",
        Rc::strong_count(&leaf), // 1, still from initial creation.
        Rc::weak_count(&leaf), // 0
    );
}
```
...
## Chapter 16 Concurrency in Rust - Creating Threads (28)

- [Link](https://www.youtube.com/watch?v=06WcsNPUNC8&list=PLai5B987bZ9CoVR-QEIN9foz4QCJ0H2Y8&index=28)

- _concurrency_ is a concept about different parts of the program running independently and/or at the same time, just to keep it simple.
- Rust aims to have safe concurrency by using ownership model and type system and capture errors at compile time.
- Rust has various tools to handle concurrency since it's a lower programming language.

### Using Threads

- Most current operating systems runs multiple processes at once.
- Each process is how an application's code is executed.
- Om each application, parts of codes can run independently, each is a _thread_.
- Splitting application in multiple threads can be efficient but introduces complexity.
- Developer doesn't have control the order of how code is run.
- _Race conditions_ is where code competes for resource access in an inconsistent order.
- _Deadlocks_ are where 2 threads waiting for a resource that is simultaneously being accessed. Both wait infinitely for the resource to become available.
- _Execution Order_ is a non-deterministic operation and bugs appear in some cases but not others. It's hard to reproduce and debug.
- Developers should be mindful when executing multi-threads.

## 2 Types of Threads

- 1st: OS Threads, 1-to-1 threads, native threads, system threads.
- 2nd: Green threads, M-to-N threads, user threads, program threads.
- Each model has trade-offs but most important to rust is _Run-time support_.
- Meaning that Rust wants to have the smallest size binary possible by not having a lot of green threads.
- Rust implements OS threads by default but we need libraries if we want to do green threads.

- _OS Threads_ are mapped 1-to-1 to the OS threads.
- Many OS have this implementation as a standard.

- _Green threads_ might have their own special implementation.
- It's not a 1-to-1 mapping or M-to-N Model.
- Does not have map to OS threads.
- Can have 20 green threads that map to 5 OS threads.
...
```rust
// ===Spawn thread terminates when Main terminates.
// Even if spawn thread didn't finish.
us std::{thread, time::Duration};
...
### Using handle and join

```rust
// ===Letting Spawn Thread finish.
us std::{thread, time::Duration};

fn main() {
  // handle is of JoinHandle type.
  ...
  // In this case, main is prevented from exiting and spawn can finish.
  // join() returns a Result type and needs unwrapping.
  handle.join().unwrap();
}
...
fn main() {
  ...
  // Calling here will completely block main thread
  // until the handle (spawn) thread has finished completely.
...
- We move ownership to inside the closure.
- We have to do this because when we spawn the thread, we don't know if v could still exist and how long our thread runs for.
- Taking ownership prevents any actions that can act on our variable until handle.join() runs.
...
fn main() {
  ...
  // We can't use references because it can mutate.
  // So add `move` so the closure can take ownership of v.
  // Also, v can no longer be used in the main thread.
  let handle = thread::spawn(move || {
    println!("here's a vector: {:?}", v);
  })

  // Problematic if run here.
  // drop(v);
